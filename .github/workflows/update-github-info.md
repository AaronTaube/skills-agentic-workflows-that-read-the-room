...
4. Update `site/content/github-info.md` with the most useful current information for developers. Keep summaries short, practical, and grounded in the fetched sources. Include the source URL for each addition or change.
5. Use the `create-pull-request` safe output to open a pull request containing the update. Make the title and body explain what changed and which GitHub Blog or Changelog sources support it, and state that the pull request is ready for Mona to review.
